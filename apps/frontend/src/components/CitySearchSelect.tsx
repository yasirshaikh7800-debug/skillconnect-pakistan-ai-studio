    ...
    <div className={`relative inline-block text-left ${className}`} ref={dropdownRef}>
      ...
      <button
        ...
        className={`flex items-center space-x-1.5 transition-all text-slate-900 dark:text-white ${
          compact
            ? 'px-3 py-1.5 rounded-xl bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-xs font-semibold hover:border-blue-600/50 cursor-pointer'
            : 'w-full px-3.5 py-2.5 rounded-xl bg-slate-100/90 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-xs font-semibold justify-between hover:border-blue-600/50 shadow-sm cursor-pointer'
        } ${buttonClassName}`}
      >
        <div className="flex items-center space-x-2 truncate">
          <MapPin className={`${compact ? 'w-3.5 h-3.5' : 'w-4 h-4'} text-blue-600 dark:text-blue-400 flex-shrink-0`} />
        ...
        <div className="absolute left-0 mt-2 w-72 sm:w-80 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-2xl z-50 overflow-hidden animate-fadeIn space-y-2 p-3">
          ...
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-9 pr-8 py-2 text-xs rounded-xl bg-slate-100 dark:bg-slate-800 text-slate-900 dark:text-white placeholder-slate-400 border border-slate-200 dark:border-slate-700 focus:outline-none focus:border-blue-600"
            />
            {searchQuery && (
              <button
                type="button"
                    ...
                    <button
                      ...
                      className={`px-2.5 py-1 rounded-lg text-[11px] font-medium transition-all cursor-pointer ${
                        isSelected
                          ...
                          : 'bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 hover:bg-blue-50 dark:hover:bg-slate-700 hover:text-blue-600 dark:hover:text-blue-400'
                      }`}
                    >
                      {popCity}
                    </button>
