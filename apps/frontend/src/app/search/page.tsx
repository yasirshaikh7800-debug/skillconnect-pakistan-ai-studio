              ...
              <div className="pt-3 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between text-xs">
                ...
                <div className="flex space-x-2">
                  ...
                  <button
                    ...
                    className="px-3 py-1.5 rounded-lg bg-emerald-600 text-white font-semibold flex items-center space-x-1 hover:bg-emerald-700 transition-colors"
                  >
                  ...
                  <Link
                    ...
                    className="px-3 py-1.5 rounded-lg bg-brand-600 text-white font-semibold flex items-center space-x-1 hover:bg-brand-500 transition-colors"
                  >
                    <span>Book</span>
                  </Link>
                </div>
              ...
              {activeChatProvider === provider.id && (
                <div className="p-3 bg-slate-50 dark:bg-slate-800/80 rounded-xl space-y-2 text-xs border border-slate-200 dark:border-slate-700">
                  <div className="flex justify-between items-center font-bold text-slate-800 dark:text-slate-200">
                    ...
                    <button onClick={() => setActiveChatProvider(null)} className="text-slate-400 hover:text-slate-600 dark:hover:text-slate-200">
                      ✕
                    </button>
                  </div>
                  ...
                  <div className="space-y-2 max-h-40 overflow-y-auto pr-1">
                    ...
                    {(chatMessages[provider.id] || []).map((msg, idx) => (
                      ...
                      >
                        <p>{msg.text}</p>
                        <span className={`text-[10px] block mt-1 ${msg.sender === 'user' ? 'text-emerald-200' : 'text-slate-400'}`}>
                  ...
                  <div className="flex space-x-2 pt-1">
                    <input
                      ...
                      className="flex-1 p-2 rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-900 dark:text-white focus:outline-none focus:ring-1 focus:ring-emerald-500"
                    />
                    <button
                      onClick={() => handleSendMessage(provider.id, provider.user?.profile?.firstName || 'Worker')}
                      className="px-3 py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg font-bold transition-colors"
                    >
                      Send
                    </button>
                  </div>
